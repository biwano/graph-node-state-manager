...
export async function deployProjectContractsTask(
  projectName: string,
  projectDir: string,
): Promise<void> {
  ...
  console.debug(
    `🧹 Cleared existing contracts from config for project: ${projectName}`,
  );
  ...
  console.info(
    `🚀 Deploying ${subgraphData.dataSources.length} data source contracts for project: ${projectName}`,
  );
  ...
  for (const contract of subgraphData.dataSources) {
    ...
    if (!(await exists(scriptPath))) {
      console.warn(
        `Deployment script not found for ${contract.name}: ${scriptPath}`,
      );
      continue;
    }
  ...
  console.info(
    `✅ Data source contracts deployed successfully for project: ${projectName}.`,
  );
}
